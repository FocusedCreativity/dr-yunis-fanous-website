  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      if (mobile) {
        ...
        // Header ~56px, footer ~80px, padding ~32px, instructions ~40px = ~208px
        const availableHeight = windowHeight - 208;
        // Page aspect ratio is 1:1.4
        const widthFromHeight = availableHeight / 1.4;
        // Page should be almost full width on mobile (single page)
        ...
        // Desktop: Make the book BIG - fill most of the screen (shows 2 pages)
        // Header is ~60px, footer is ~100px, instructions ~40px = 200px
        const availableHeight = windowHeight - 220;
        // Page aspect ratio is 1:1.4
        // Calculate width from available height
        const widthFromHeight = availableHeight / 1.4;
        // Each page should be almost half the screen width (book shows 2 pages)
        // So single page width = ~45% of screen
        const widthFromScreen = windowWidth * 0.45;
        // Use the constraint that fits, but be generous
        const baseWidth = Math.min(widthFromHeight, widthFromScreen);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#2F4F3E] to-[#1E3328] flex flex-col" dir="rtl">
      ...
      <header className="bg-[#2F4F3E]/95 backdrop-blur border-b border-[#F6F2EA]/20 px-4 py-3 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            href="/book.pdf"
            download="من_أجل_القضية_الوطنية.pdf"
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 overflow-hidden">
        ...
        {/* Document loader (hidden, just for loading) */}
        <Document
          file="/book.pdf"
        ...
        {!isLoading && numPages > 0 && (
          <Document file="/book.pdf" loading={null}>
